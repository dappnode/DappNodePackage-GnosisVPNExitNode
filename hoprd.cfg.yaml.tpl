...
session_ip_forwarding:
  target_allow_list:
    ...
    # config parser), so entrypoint.sh resolves gnosisvpn-server's container IP at startup and
    # substitutes it here - this package can't pin a static IP for it (no custom docker
    # networks allowed for non-core DAppNode packages; see README).
